# -*- coding: utf-8 -*-
__filename = 'C:\\Users\\Alex\\Documents\\fernz\\src\\templates\\checks.pynml'
...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            __backup_incompatible_grf_2075083925600 = get('incompatible_grf', __marker)

            # <Value 'incompatible_grfs' (14:48)> -> __iterator
            __token = 503
            __iterator = getitem('incompatible_grfs')
            (__iterator, ____index_2075099075024, ) = getitem('repeat')('incompatible_grf', __iterator)
            econtext['incompatible_grf'] = None
            for __item in __iterator:
                econtext['incompatible_grf'] = __item

                # <Interpolation value=<Substitution '\n    if (grf_future_status("${incompatible_grf.grfid}")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "${incompatible_grf.grfname}"));\n    }\n' (14:67)> braces_required=True translation=False at 1e3258b1128> -> __content_2075086945112
                __token = 527
                __token = 552
                __content_2075086945112 = _lookup_attr(getitem('incompatible_grf'), 'grfid')
                __content_2075086945112 = __quote(__content_2075086945112, '\x00', '&#0;', None, False)
                __token = 638
                __content_2075086945112_636 = _lookup_attr(getitem('incompatible_grf'), 'grfname')
                __content_2075086945112_636 = __quote(__content_2075086945112_636, '\x00', '&#0;', None, False)
                __content_2075086945112 = ('%s%s%s%s%s' % ('\n    if (grf_future_status("', (__content_2075086945112 if (__content_2075086945112 is not None) else ''), '")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "', (__content_2075086945112_636 if (__content_2075086945112_636 is not None) else ''), '"));\n    }\n', ))
                ...
                ____index_2075099075024 -= 1
                if (____index_2075099075024 > 0):
                    __append('')
            if (__backup_incompatible_grf_2075083925600 is __marker):
                del econtext['incompatible_grf']
            else:
                econtext['incompatible_grf'] = __backup_incompatible_grf_2075083925600
            __append('\n\n\n')
